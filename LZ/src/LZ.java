...
class LZ77v01 {
	 ...
	 */
	String rawData = "Miss Kalissippi from Mississippi is a " + "cowgirl who yells yippi when she rides her horse in "
	...
	void doIt() {
		...
		while (charCnt < rawData.length()) {
			...
			searchWindowStart = (charCnt - searchWindowLen >= 0) ? charCnt - searchWindowLen : 0;
			// Establish the end of the lookAhead window.
			lookAheadWindowEnd = (charCnt + lookAheadWindowLen < rawData.length()) ? charCnt + lookAheadWindowLen
					: rawData.length();
			...
			if (searchSubstring.indexOf(searchTarget) != -1) {
				...
				int offset = (charCnt < (searchWindowLen + matchLen)) ? charCnt - matchLoc - matchLen
		...
		System.out.println(
				" Maximum offset value of " + searchWindowLen + " char requires " + offsetBitsRequired + " bits.");
		System.out.println(" Maximum stringLen value of " + lookAheadWindowLen + " char requires "
	...
	int getBitsRequired(int value) {
		if (value < 2) {
			...
			System.out.println("Bit conversion too large, terminating program.");
			System.exit(1);
